<template>
  <div class="card flex justify-content-center">
    ...
    <Dialog v-model:visible="dialog" modal :pt="{
      root: 'border-none',
      mask: {
        ...
        <form @submit.prevent="handleSubmit">
          <div class="flex flex-column px-5 py-5 gap-4"
            ...
            <div class="inline-flex flex-column gap-2">
              <AlertMessage v-if="authStores.errorAlert" :message="authStores.errorMsg" style="max-width: 300px;" />
              <label for="email" class="text-primary-50 font-semibold">Email</label>
              <InputText type="email" v-model="userInput.email" id="email"
                class="bg-white-alpha-20 border-none p-3 text-primary-50 w-20rem"></InputText>
            </div>
            <div class="inline-flex flex-column gap-2">
              <label for="password" class="text-primary-50 font-semibold">Password</label>
              <InputText id="password" v-model="userInput.password"
                class="bg-white-alpha-20 border-none p-3 text-primary-50 w-20rem" type="password">
              </InputText>
            </div>
            <div class="flex align-items-center gap-3">
              ...
              <Button label="Sign-In" type="submit" @click="closeCallback" text
                class="p-3 w-full text-primary-50 border-1 border-white-alpha-30 hover:bg-white-alpha-10"></Button>
            </div>
          </div>
        </form>
